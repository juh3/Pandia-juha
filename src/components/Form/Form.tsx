...
const Form = () => {
  ...
  const handleSongChange: ChangeEventHandler<HTMLSelectElement> = (event) => {
    setValidationErrors(null);

    setSong(event.target.value);
  };

  const handleSaveChange: ChangeEventHandler<HTMLInputElement> = () => {
    setValidationErrors(null);
    setAllowSave(!allowSave);
  };

  const handleSongKeyChange: ChangeEventHandler<HTMLInputElement> = (event) => {
    setValidationErrors(null);
    ...
    <div className={styles.FormContainer}>
      ...
      <form className={styles.Form} onSubmit={handleSubmit}>
        <Input
          ...
          value={name}
          error={
            validationErrors?.find((error) => error.path[0] === 'name')?.message
          }
        />
        <Input
          ...
          accept="image/"
          error={
            validationErrors?.find((error) => error.path[0] === 'picture')
              ?.message
          }
        />
        ...
        <Input
          ...
          options={key_options}
          error={
            validationErrors?.find((error) => error.path[0] === 'songKey')
              ?.message
          }
        />
        <Input
        ...
        />
        <Button type="submit" isSubmitting={submitting} disabled={submitting}>
          Ilmoittaudu
        </Button>
      </form>
    </div>
